...
class _BaseTransformersCTranslate2Adapter(ModelAdapter):
    """Seq2seq adapter backed by CTranslate2 with on-demand HF->CT2 conversion."""
    ...
    def _require_ctranslate2(self):
        try:
            ...
            raise RuntimeError(
                "CTranslate2 is required for CT2 adapters. "
...
class M2MCTranslate2Adapter(_BaseTransformersCTranslate2Adapter):
    """CTranslate2 adapter for M2M100 models."""


class NLLBCTranslate2Adapter(_BaseTransformersCTranslate2Adapter):
    """CTranslate2 adapter for NLLB models."""

    def _prepare_tokenizer_languages(self, src_lang: str, tgt_lang: str) -> tuple[object, object]:
        # For NLLB CT2, keep source prefix based on src_lang while using target prefix at decode time.
        prior_src_lang = _UNSET
        if hasattr(self.tokenizer, "src_lang"):
            prior_src_lang = getattr(self.tokenizer, "src_lang")
            self.tokenizer.src_lang = src_lang
